...
namespace HomeOS.Hub.Scouts.WebCam
{
        ...
        public class WebCamScoutService : IWebCamScoutContract
        {
            VLogger logger;
            WebCamScout webCamScout;
            SafeServiceHost service;
            private bool disposed = false;
            public WebCamScoutService(string baseAddress, WebCamScout wcScout, ScoutViewOfPlatform platform, VLogger logger)
            {
                this.logger = logger;
                ...
                var contract = ContractDescription.GetContract(typeof(IWebCamScoutContract));
    ...
    [ServiceContract]
    public interface IWebCamScoutContract
    {
        [OperationContract]
