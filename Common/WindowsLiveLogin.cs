...
namespace WindowsLive
{
    ...
    public class WindowsLiveLogin
    {
        ...
        public byte[] EncryptClearData(byte[] clearData, byte[] Key, byte[] IV)
        {
            byte[] encryptedData;
        ...
        public string EncryptClearText(string clearText, string Password)
        {
            ...
            byte[] encryptedData = EncryptClearData(clearBytes,
        ...
        /*
        static byte[] encodeu64(string s)
        {
            byte[] b = null;
            if (s == null) { return b; }
            s = HttpUtility.UrlEncode(s);
            ...
            try
            {
                b =  GetBytes(Convert.ToBase64String(GetBytes(s)));
            }
            catch (Exception e)
            {
                debug("Error: u64: Base64 conversion error: " + s + ", " + e);
        ...
        public string EncodeString(string str)
        {
            ...
            const int ivLength = 16;
            /*
            byte[] ivAndEncryptedalue = encodeu64(str);
            ...
            try
            {
                ...
                memStream.Read(iv, 0, ivLength);
                aesAlg.IV = iv;
                cStream = new CryptoStream(memStream, aesAlg.CreateEncryptor(), CryptoStreamMode.Write);
                cStream.Write(GetBytes(str), 0, str.Length);
                //sReader = new StreamReader(cStream, Encoding.ASCII);
                encodedValue = GetString(memStream.ToArray());
            }
            catch (Exception e)
            {
                debug("Error: DecodeToken: Decryption failed: " + e);
