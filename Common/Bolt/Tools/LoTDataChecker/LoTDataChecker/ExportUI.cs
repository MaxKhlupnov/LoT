...
namespace HomeOS.Hub.Common.Bolt.Tools.LotDataChecker
{
    class ExportUI
    {
        ...
        public Dictionary<IKey, Tuple<IValue, DateTime>> GetLatest(HashSet<IKey> keys)
        {
            if (datastream == null)
                return null;

            Dictionary<IKey, Tuple<IValue, DateTime>> retval = new Dictionary<IKey, Tuple<IValue,DateTime>>();

            foreach (IKey key in keys)
            {
                Tuple<IValue, long> tupleData = datastream.GetLatest(key);
                if (null != tupleData)
                {
                    retval[key] = new Tuple<IValue, DateTime>(tupleData.Item1, (new DateTime(tupleData.Item2).ToLocalTime()));
                }
            }
            return retval;
        }
